...
impl DecodingHyperGraph {
    ...
    pub fn find_valid_subgraph(&self, edges: &FastIterSet<EdgePtr>, vertices: &FastIterSet<EdgePtr>) -> Option<Subgraph> {
    ...
    pub fn is_valid_cluster(&self, edges: &FastIterSet<EdgePtr>, vertices: &FastIterSet<VertexIndex>) -> bool {
    ...
    pub fn get_edge_neighbors(&self, edge_index: EdgePtr) -> &Vec<VertexIndex> {
        self.model_graph.get_edge_neighbors(edge_index)
    }

    pub fn get_vertex_neighbors(&self, vertex_index: VertexIndex) -> &Vec<EdgePtr> {
