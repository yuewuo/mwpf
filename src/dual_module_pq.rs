    ...
    Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Clone,
{
    ...
    pub fn new_partitioned(partitioned_initializer: &PartitionedSolverInitializer, partition_id: usize) -> Self {
        // println!("///////////////////////////////////////////////////////////////////////////////");
        ...
        let global_time = ArcManualSafeLock::new_value(Rational::zero(), (0, 0));
        ...
        for (hyper_edge, edge_index) in partitioned_initializer.weighted_edges.iter() {
            ...
            let edge_ptr = EdgePtr::new_value(
                Edge {
        ...
        Self {
            ...
            unit_active: ArcManualSafeLock::new_value(false, (0, 0)), // false by default, to be updated later when edge_growth are calcualted
            tuning_start_time: None,
            total_tuning_time: None,
            negative_weight_sum: Default::default(),
            negative_edges: Default::default(),
    ...
    pub fn new_seperate_unit(new_seperate_initializer: &PartitionedSolverInitializer, partition_id: usize) -> Self {
        // initializer.sanity_check().unwrap();
        ...
        for (hyperedge, _) in new_seperate_initializer.weighted_edges.iter() {
            let edge_ptr = EdgePtr::new_value(
                ...
                (partition_id, edges.len() as EdgeIndex),
            );
            for &vertex_index in hyperedge.vertices.iter() {
                vertices[vertex_index as usize].write().edges.push(edge_ptr.downgrade());
            }
        ...
        Self {
            ...
            unit_active: ArcManualSafeLock::new_value(false, (0, 0)), // used only for parallel implementation
            tuning_start_time: None,
            total_tuning_time: None,
            negative_weight_sum: Default::default(),
            negative_edges: Default::default(),
