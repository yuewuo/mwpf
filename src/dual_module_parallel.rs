            ...
            (0..unit_count)
                ...
                .map(|unit_index| {
                    ...
                    let mut dual_module: DualModulePQGeneric<Queue> =
                        DualModulePQGeneric::new_partitioned(&partitioned_initializers[unit_index], unit_index);

                    DualModuleParallelUnitPtr::new_value(
    ...
    Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Send + Sync + Clone,
{
    pub fn new_seperate_config(
        ...
        let mut dual_module: DualModulePQGeneric<Queue> =
            DualModulePQGeneric::new_seperate_unit(&partitioned_solver_initializer, partition_id);
        DualModuleParallelUnit {
