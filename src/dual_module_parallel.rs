        ...
        thread_pool.scope(|_| {
            (0..unit_count)
                ...
                .map(|unit_index| {
                    ...
                    let mut dual_module: DualModulePQGeneric<Queue> = DualModulePQGeneric::new_partitioned(&partitioned_initializers[unit_index]);
...
where Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Send + Sync + Clone,
{
    pub fn new_seperate_config(initializer: &SolverInitializer,
        ...
        let mut dual_module: DualModulePQGeneric<Queue> = DualModulePQGeneric::new_seperate_unit(&partitioned_solver_initializer); 
...
pub mod tests {
    ...
    fn dual_module_parallel_test_1() 
    {
        ...
        // the result subgraph
        let subgraph_weak = vec![dual_module.get_edge_ptr(15).downgrade(), dual_module.get_edge_ptr(20).downgrade()];
        let subgraph = subgraph_weak.iter().map(|e| e.upgrade_force().read_recursive().edge_index).collect::<Vec<_>>();
        visualizer
