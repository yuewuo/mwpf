...
pub struct PrimalModuleParallel {
    ...
    pub last_solve_start_time: ArcManualSafeLock<Instant>,
}
...
impl PrimalModuleParallel {
    pub fn new_config(
        ...
        thread_pool.scope(|_| {
            (0..unit_count)
                ...
                .map(|unit_index| {
                    ...
                    PrimalModuleParallelUnitPtr::new_value(
        ...
        Self {
            ...
            last_solve_start_time: ArcManualSafeLock::new_value(time, (0, 0)),
        }
    }
}
...
impl PrimalModuleParallelUnitPtr {
        ...
        Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Send + Sync + Clone,
    {
        ...
        if !primal_unit.is_solved {
            ...
            if let Some(callback) = callback.as_mut() {
                callback(
                    &primal_unit.interface_ptr,
                    &dual_module_ptr.write().deref_mut(),
                    &primal_unit.serial_module,
                    None,
                );
            }
        }
        ...
        for (adjacent_unit_ptr, is_fused) in self.adjacent_parallel_units.iter_mut() {
            if *is_fused {
                ...
                *is_fused = true;
                ...
                let adjacent_dual_unit_ptr =
                    &parallel_dual_module.units[adjacent_unit_ptr.upgrade_force().read_recursive().unit_index];
                self_dual_unit
                    .adjacent_parallel_units
                    .push(adjacent_dual_unit_ptr.downgrade());
            }
        }
        drop(self_dual_unit);
    }
...
impl PrimalModuleParallel {
        ...
        Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Send + Sync + Clone,
    {
        if let Some(visualizer) = visualizer {
            ...
            self.parallel_solve_step_callback(syndrome_pattern, parallel_dual_module, callback);
            let last_unit = self.units.last().unwrap().read_recursive();
            visualizer
                .snapshot_combined(
                    "solved".to_string(),
...
impl PrimalModuleImpl for PrimalModuleParallel {
    ...
    fn subgraph(
        ...
        // No Locks: Each thread accumulates its results locally and then combines them at the end, which avoids the need for locking.
        // Efficient Parallelization: The threads can work independently and use Rayon’s reduce to combine results efficiently after the work is done.
        let (subgraph, internal_subgraph, flip_edge_indices): (Vec<usize>, Vec<EdgeWeak>, HashSet<_>) = self
            .units
            .par_iter()
            .map(|unit_ptr| {
                let mut unit = unit_ptr.write();
                let interface_ptr = unit.interface_ptr.clone();
                let unit_output_subgraph = unit.subgraph(&interface_ptr, dual_module);

                // Return the result of this unit’s work as a tuple
                (
                    unit_output_subgraph.subgraph.clone(),
                    OutputSubgraph::get_internal_subgraph(&unit_output_subgraph).clone(),
                    unit_output_subgraph.flip_edge_indices,
                )
            })
            .reduce(
                || (Vec::new(), Vec::new(), HashSet::new()), // Identity for reduction
                |(mut subgraph_acc, mut internal_subgraph_acc, mut flip_edge_indices_acc),
                 (subgraph_res, internal_subgraph_res, flip_edge_indices_res)| {
                    subgraph_acc.extend(subgraph_res);
                    internal_subgraph_acc.extend(internal_subgraph_res);
                    flip_edge_indices_acc.extend(flip_edge_indices_res);
                    (subgraph_acc, internal_subgraph_acc, flip_edge_indices_acc)
                },
            );

        OutputSubgraph::new(subgraph, flip_edge_indices, internal_subgraph)
        ...
        //     let interface_ptr = unit.interface_ptr.clone();
        //     let unit_output_subgraph = unit.subgraph(&interface_ptr, dual_module);
        //     subgraph.extend(unit_output_subgraph.subgraph.clone());
        //     internal_subgraph.extend(OutputSubgraph::get_internal_subgraph(&unit_output_subgraph).clone());
        //     flip_edge_indices.extend(unit_output_subgraph.flip_edge_indices);
        // }
        // OutputSubgraph::new(subgraph, flip_edge_indices, internal_subgraph)
    }

    // TODO: fix the lower bound!
    fn subgraph_range(
        &mut self,
        interface: &DualModuleInterfacePtr,
        ...
        let upper: Rational = internal_subgraph
            .par_iter()
            .map(|edge_weak| edge_weak.upgrade_force().read_recursive().weight.clone())
            .sum();
        let lower: Rational = (0..self.units.len())
            .into_par_iter()
            .map(|unit_index| {
                let primal_unit = self.units[unit_index].read_recursive();
                primal_unit.interface_ptr.sum_dual_variables() // Accumulate the sum
            })
            .sum();

        // Add the result of `get_negative_weight_sum` to `lower`
        let lower = lower + dual_module.get_negative_weight_sum();
        let weight_range = WeightRange::new(lower, upper);
        ...
        //     // lower += primal_unit.interface_ptr.sum_dual_variables() + dual_module.units[unit_index].get_negative_weight_sum();
        //     lower += primal_unit.interface_ptr.sum_dual_variables();
        // }
        // lower += dual_module.get_negative_weight_sum();
    ...
    fn generate_profiler_report(&self) -> serde_json::Value {
        let event_time_vec: Vec<_> = self
            .units
            .iter()
            .map(|ptr| ptr.read_recursive().event_time.clone())
            .collect::<Vec<_>>();
        json!({
...
impl PrimalModuleImpl for PrimalModuleParallelUnit {
    ...
    fn load<D: DualModuleImpl>(&mut self, interface_ptr: &DualModuleInterfacePtr, dual_module: &mut D, partition_id: usize) {
        self.serial_module.load(interface_ptr, dual_module, partition_id);
    }
    ...
    fn subgraph_range(
        ...
        self.serial_module.subgraph_range(interface, dual_module)
    }
}
...
pub mod tests {
    ...
    pub fn primal_module_parallel_basic_standard_syndrome(
        ...
        final_dual: Weight,
        plugins: PluginVec,
    ) -> (PrimalModuleParallel, impl DualModuleImpl + MWPSVisualizer) {
        ...
        let initializer = &model_graph.initializer.clone();
    ...
    pub fn primal_module_parallel_basic_standard_syndrome_optional_viz<Queue>(
        defect_vertices: Vec<VertexIndex>,
        final_dual: Weight,
        mut dual_module: DualModuleParallel<DualModulePQGeneric<Queue>, Queue>,
        ...
        Queue: FutureQueueMethods<Rational, Obstacle> + Default + std::fmt::Debug + Send + Sync + Clone,
    {
        ...
        // assert_eq!(weight_range.lower, weight_range.upper, "lower and upper of weight range do not match");
    ...
    pub fn primal_module_parallel_evaluation_qec_playground_helper(
        ...
        let model_graph = code.get_model_graph();
        let initializer = &model_graph.initializer.clone();
        let (partition_config, no_per_layer) =
            graph_time_partition(&initializer, &code.get_positions(), &defect_vertices, split_num);
        let partition_info = partition_config.info();
        ...
        // create primal module
        let mut primal_config = PrimalModuleParallelConfig { ..Default::default() };
        primal_config.thread_pool_size = thread_pool_size;
        let primal_module = PrimalModuleParallel::new_config(
            &model_graph.initializer,
            &partition_info,
            primal_config.clone(),
    ...
    fn primal_module_parallel_circuit_level_noise_qec_playground_1() {
        ...
        primal_module_parallel_evaluation_qec_playground_helper(
            ...
            visualize_filename,
            defect_vertices,
            Rational::from(8.788898309344878), // dummy value
            vec![],
            2,
            2,
        );
    ...
    /// test solver on circuit level noise with random errors, split into 2
    #[test]
    fn primal_module_parallel_circuit_level_noise_qec_playground_serial_2() {
        // cargo test -r primal_module_parallel_circuit_level_noise_qec_playground_serial_2 -- --nocapture
        let config = json!({
            "code_type": qecp::code_builder::CodeType::RotatedPlanarCode,
            "nm": 2000,
        });
        ...
        let visualize_filename = "primal_module_parallel_circuit_level_noise_qec_playground_serial_2.json".to_string();
    ...
    /// test solver on circuit level noise with random errors, split into 4
    #[test]
    fn primal_module_parallel_circuit_level_noise_qec_playground_serial_3() {
        // cargo test -r primal_module_parallel_circuit_level_noise_qec_playground_serial_3 -- --nocapture
        let config = json!({
            "code_type": qecp::code_builder::CodeType::RotatedPlanarCode,
            "nm": 500,
        });
        ...
        let visualize_filename = "primal_module_parallel_circuit_level_noise_qec_playground_serial_3.json".to_string();
        use crate::primal_module_serial::tests::primal_module_serial_basic_standard_syndrome;
        primal_module_serial_basic_standard_syndrome(
            code,
            visualize_filename,
    ...
    //     let mut primal_config = PrimalModuleParallelConfig {..Default::default()};
    //     primal_config.timeout = 7.0;
    //     let primal_module = PrimalModuleParallel::new_config(&model_graph.initializer, &partition_info, primal_config.clone(), plugins, model_graph.clone());
    ...
    //     let new_dual_module: DualModuleParallelUnit<DualModulePQGeneric<FutureObstacleQueue<Rational>>, FutureObstacleQueue<Rational>> = DualModuleParallelUnit::new_seperate_config(&new_initializer, &new_partition_info, new_dual_module_parallel_config);
    ...
    //     let new_primal_module = PrimalModuleParallelUnit::new_config(&new_initializer, &new_partition_info, new_primal_config.clone(), new_plugins, new_model_graph);
    ...
    //     new_final_dual: Weight,
    //     mut dual_module: DualModuleParallel<DualModulePQGeneric<Queue>, Queue>,
    //     mut new_dual_module: DualModuleParallelUnit<DualModulePQGeneric<Queue>, Queue>,
    //     mut primal_module: PrimalModuleParallel,
    ...
    //     let useless_interface_ptr = DualModuleInterfacePtr::new(new_decoding_graph.model_graph.clone());
    //     let (subgraph, weight_range) = new_primal_module.subgraph_range(&useless_interface_ptr, &mut new_dual_module);
    ...
    //     // let new_weight = 1;
    //     let new_code = CodeCapacityPlanarCode::new(7, 0.1);
    ...
    //     let mut new_primal_config = PrimalModuleParallelConfig {..Default::default()};
    //     new_primal_config.timeout = 7.0;
    //     let new_primal_module = PrimalModuleParallelUnit::new_config(&new_initializer, &new_partition_info, new_primal_config.clone(), new_plugins, new_model_graph.clone());
    //     let new_primal_module_ptr = PrimalModuleParallelUnitPtr::new_value(new_primal_module);
