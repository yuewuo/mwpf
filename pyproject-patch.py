...
static_patches = [
    ...
    (
        ...
        [
            ...
            ("SinterMWPFDecoder", "SinterMWPF$Name$Decoder", 5),
            ("SinterHUFDecoder", "SinterHUF$Name$Decoder", 1),
            ("SinterSingleHairDecoder", "SinterSingleHair$Name$Decoder", 1),
            ("MwpfCompiledDecoder", "Mwpf$Name$CompiledDecoder", 5),
            ("SinterBPMWPFDecoder", "SinterBPMWPF$Name$Decoder", 2),
            ("BPMWPFCompiledDecoder", "BPMWPF$Name$CompiledDecoder", 3),
        ],
    ),
    (
...
if __name__ == "__main__":
    assert (
        len(sys.argv) == 3
    ), "Usage: python pyproject-patch.py [rational|incr] [dry|apply|revert]"
    assert sys.argv[1] in ["rational", "incr"]
    if sys.argv[2] == "dry":
